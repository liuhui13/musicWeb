const song = {
    state:{
        ...
        url: '',                //歌曲地址
        id: '',                  //歌曲id
        playButtonUrl: '#icon-liuhui-bofang',   //播放状态的图标
        duration: [],                //音乐时长
        curTime:  0,                //当前音乐的播放位置
        changeTime: 0,              //指定播放时刻
        title: '',                  //歌名
        ...
        volume: 15,                  //音量
        playOrderUrl: '#icon-liuhui-shunxu', //播放顺序图标，默认为顺序播放
        playOrder:  1, //播放顺序，默认为1顺序播放,2为随机播放，0为单曲播放
        numPlay: 0, //播放数量
    ...
    getters: {
        ...
        playOrder: state => {
            ...
            if(!playOrder){
                playOrder = JSON.parse(window.sessionStorage.getItem('playOrder')||null); 
            }
            return playOrder;
        },
        playOrderUrl: state => {
            let playOrderUrl = state.playOrderUrl;
            if(!playOrderUrl){
                playOrderUrl = JSON.parse(window.sessionStorage.getItem('playOrderUrl')||null); 
            }
            return playOrderUrl;
        },
        duration: state => {
        ...
        numPlay: state => {
            let numPlay = state.numPlay;
            if(!numPlay){
                numPlay = JSON.parse(window.sessionStorage.getItem('numPlay')||null); 
            }
            return numPlay;
        },  
    },
    mutations: {
        ...
        setPlayOrder: (state,playOrder) => {
            ...
            window.sessionStorage.setItem('playOrder',JSON.stringify(playOrder));
        },
        setPlayOrderUrl: (state,playOrderUrl) => {
            state.playOrderUrl = playOrderUrl;
            window.sessionStorage.setItem('playOrderUrl',JSON.stringify(playOrderUrl));
        },
        ...
        setNumPlay: (state,numPlay) => {
            state.numPlay = numPlay;
            window.sessionStorage.setItem('numPlay',JSON.stringify(numPlay));
        },      
    }
}
